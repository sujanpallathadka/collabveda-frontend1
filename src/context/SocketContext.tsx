...
const SocketProvider = ({ children }: { children: ReactNode }) => {
    ...
    const handleFileLocked = useCallback(({ fileId, userId }: { fileId: string; userId: string }) => {
        setLockedFiles((prev) => ({ ...prev, [fileId]: userId }))
    }, [])

    const handleFileUnlocked = useCallback(({ fileId }: { fileId: string }) => {
        setLockedFiles((prev) => {
            const updatedLocks = { ...prev }
            delete updatedLocks[fileId]
            return updatedLocks
        })
    }, [])
    ...
    useEffect(() => {
        ...
        socket.on("file-locked", handleFileLocked)
        socket.on("file-unlocked", handleFileUnlocked)
